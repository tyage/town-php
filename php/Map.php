	...
	static function makeBigMapTable(){
		...
		for($y=Ini::$mapVector["y"];$y>=1;$y--){
			...
			for($x=1;$x<=Ini::$mapVector["x"];$x++){
				...
				$googleMap = "http://maps.google.com/staticmap?center={$googleMapY},{$googleMapX}&span=2,2&size=640x640&sensor=false&format=jpg&maptype=satellite&key=ABQIAAAADyX-jOA8X0olmqI2eqdQuxS8YCOCVoYBzf0FMVtac3Xci-xHrRQ2NTrx8kYuj53DhyLOpf3aVxPrLg";
				if(!empty($map)){
