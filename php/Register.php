...
EOF;
	...
	static function start(){
		...
		if($double){
			...
			Mail::write(Ini::$admin_id ,array("type" => "報告","name" => $_POST["name"],"title" => "多重の可能性があります。","message" => "{$_POST["name"]}さんはID:{$double}と多重である可能性があります。"));
		}else{
			Mail::write(Ini::$admin_id ,array("type" => "報告","name" => $_POST["name"],"title" => "登録しました。","message" => "{$_POST["name"]}さんが登録しました。"));
		}
